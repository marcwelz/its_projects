...
public class Main {
    public static void main(String[] args) {
        ...
        persons[100] = new Person("AB",  "AA", 1.54, 42, 34);
        persons[101] = new Person("AA",  "AA", 1.66, 39, 27);

        PersonComparator pc = new PersonComparator();
        HeightComparator hc = new HeightComparator();
        ShoesizeComparator sc = new ShoesizeComparator();
        AgeComparator ac = new AgeComparator();

        System.out.println("---Start---");
        // Limited to 20
        Stream.of(persons).limit(20).forEach(System.out::println);
        Arrays.sort(persons, pc);
        System.out.println("---Nname--");
        Stream.of(persons).limit(20).forEach(System.out::println);

        Arrays.sort(persons, ac);
        System.out.println("---Age---");
        Stream.of(persons).limit(20).forEach(System.out::println);

        Arrays.sort(persons, hc);
        System.out.println("---Height---");
        Stream.of(persons).limit(20).forEach(System.out::println);

        Arrays.sort(persons, sc);
